...
def Binarize(tensor,quant_mode='det'):
    if quant_mode=='det':
        ...
        return tensor.add_(1).div_(2).add_(torch.rand(tensor.size(),device=torch.device('cuda')).add(-0.5)).clamp_(0,1).round().mul_(2).add_(-1)

def Ternarize(tensor,quant_mode='det'):
    maxvalue = torch.max(tensor.abs())
    result = tensor.mul(1.49/maxvalue).round()
    return result
